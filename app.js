...
io.on('connection', function (socket) {
    ...
    socket.on('orderDroppedOff', function (orderId) {
        let order = data.orders[orderId];
        console.log("Order",orderId,"was dropped off");
        if (order.orderDetails.origin === 0) { // if it's in a car and from the outside
            if (order.orderDetails.express) {
        ...
        } else if (order.orderDetails.origin === 1) {
            data.orderDropOff(orderId); // drop it off
            io.emit('orderDroppedOff', orderId);
        }
        // send updated info to all connected clients, note the use of io instead of socket
    });
